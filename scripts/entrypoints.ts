...
export function generateEntrypoints(inputs: string[]) {
  ...
  const pkgJson: PackageJson = JSON.parse(fs.readFileSync(path.resolve('package.json'), 'utf8'));
  ...
  inputs
    ...
    .forEach((i) => {
      ...
      const resolvedImport = path.join(...Array(importDepth).fill('..'), 'dist', importPath);
      // index.js
